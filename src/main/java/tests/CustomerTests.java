...
public class CustomerTests  {
	...
	public void TC_005_Customer_Sign_in() throws MalformedURLException, InterruptedException {
		AppiumServerManager.startAppiumServer();
		driver = AppiumDriverManager.getDriver();
		String actual = Library.customer_SignIn_Details_nulldata(driver);
		Assert.assertEquals(actual,ErrorMessages.nullvaluesigninEmailErrormessage);	
	}

	@Test(priority = 6, enabled = false, description = "Customer SignIn with invalid data", dataProvider ="CustomerSignininvaliddata", dataProviderClass = utils.DataProviderClass.class)
